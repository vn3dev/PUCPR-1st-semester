...
# fluxo:
# 1. obtém tipo da URL (ex: A-, A+, etc.)
...
# 6. compara o tipo da URL com os do json para encontrar o registro correspondente. Se n, retorna 404
...
@sangue_bp.put("/sangue/atualizar/<tipo>")
def update_sangue(tipo):
    ...
    # normaliza tipo da URL (ex: A-, A+, etc.)
    tipo = str(tipo).upper()
    ...
    # compara o tipo da URL com os do json
    registro = next((item for item in sangue if item.get('tipo') == tipo), None)
    if registro is None:
        return jsonify({"error": "Registro não encontrado para tipo fornecido"}), 404
